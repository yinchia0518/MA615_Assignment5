...
tox1 %>%
  filter(!is.na(`Values for LD50 on rats`)) %>%
  ggplot(aes(x = Name, y = `Values for LD50 on rats`)) +
  geom_bar(stat = "identity", fill = "steelblue") + coord_flip() +
  labs(x = "Chemical", title = "Aver. Quantity of chemicals (mg/kg) causing mortality in rats")

tox1 %>%
  filter(!is.na(`Values for LC50 on earthworms`)) %>%
  ggplot(aes(x = Name, y = `Values for LC50 on earthworms`)) +
  geom_bar(stat = "identity", fill = "steelblue") + coord_flip() +
...
coeff <- coeff %>%
  rename("Soil Adsorption coeff" = Soil.Absorbed.Coeff..L.Kg.) %>%
